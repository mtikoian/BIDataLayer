...
Truncate Table as_dwh.EDW.Fact_SALES_Open_Order;
Go

DBCC CHECKIDENT ([AS_DWH.EDW.FACT_SALES_OPEN_ORDER], RESEED, 1);
go
...
left join as_dwh.edw.Dim_Product_Combined_V p
on f.Item_Number = p.GS_Item_Number
...
Left Join as_dwh.[EDW].[GS_Dim_CUSTOMER] c
on f.Oracle_Cust_Num = c.Account_Number

Left Join as_stage.dbo.[stg_Dim_CUSTOMER_Location_gs] cl
on f.Oracle_Cust_Num = cl.Manman_Location_Number
